...
const ThemesInstallable = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/themes/install-from-github`, {
        method: 'POST',
      ...
      setSuccess('Thème installé avec succès !');
      setRepo('');
      router.refresh();
    } catch (err) {
      setError(err.message);
      console.error("Erreur lors de l'installation du thème :", err);
    } finally {
      setInstalling(false);
    }
  };
  ...
  return (
    <div className="card card-body bg-secondary">
      <h5 className="card-title">Installer un thème depuis GitHub</h5>
      <p className="text-muted">
        Le dépôt doit être public et avoir au moins une release GitHub. Un asset nommé{' '}
        <code>theme.zip</code> est utilisé s&apos;il existe, sinon l&apos;archive source de la
